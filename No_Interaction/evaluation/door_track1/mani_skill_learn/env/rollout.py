...
class BatchRollout:
    def __init__(self, env_cfg, num_procs=20, synchronize=True, reward_only=False, **kwargs):
        ...
        for i in range(self.n):
            if synchronize:
                self.workers.append(NormalWorker(Rollout, i, env_cfg, reward_only=reward_only, **kwargs))
            else:
                self.workers.append(TorchWorker(Rollout, i, env_cfg, reward_only=reward_only, **kwargs))
