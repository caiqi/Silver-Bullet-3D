...
@REPLAYS.register_module()
class ReplayMemoryRGBInt(ReplayMemory):
    def sample(self, batch_size):
        batch_idx = np.random.randint(low=0, high=len(self), size=batch_size)
        samples = sample_element_in_dict_array(self.memory, batch_idx)
        rgb = samples['obs']['pointcloud']['rgb'].astype(np.float32) / 255.0
        samples['obs']['pointcloud']['rgb'] = rgb
        # xyz = samples['obs']['pointcloud']['xyz'].astype(np.float32)
        # samples['obs']['pointcloud']['xyz'] = xyz
        return samples
...
@REPLAYS.register_module()
class ReplayMemoryRGBIntNextActions(ReplayMemory):
    def sample(self, batch_size):
        batch_idx = np.random.randint(low=0, high=len(self), size=batch_size)
        samples = sample_element_in_dict_array(self.memory, batch_idx)
        rgb = samples['obs']['pointcloud']['rgb'].astype(np.float32) / 255.0
            ...
            for init_buffer in init_buffers:
                for item in init_buffer:
                    if cnt >= num_trajs_per_demo_file and num_trajs_per_demo_file != -1:
                        break
                    item = {key: item[key] for key in buffer_keys}
                    next5_actions = item['actions']
                    next5_actions = np.concatenate((next5_actions, np.repeat(next5_actions[-1:, :], 5, axis=0)), axis=0)
                    next5_actions = next5_actions[5:, :]
                    item['actions'] = np.concatenate((item['actions'], next5_actions), axis=-1)
                    self.push_batch(**item)
                    cnt += 1
        print(f'Num of buffers {len(init_buffers)}, Total steps {self.running_count}')
